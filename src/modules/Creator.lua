...
local cloneref = function(instance) return instance end
...
local IconAliases = {
	...
	["sfsymbols:sunMaxFill"] = "sun-medium",
}
...
local function normalizeIconName(iconName)
	...
	return IconAliases[iconName] or iconName
end
...
function Creator.Icon(iconName)
	local normalizedName = normalizeIconName(iconName)
	local iconAsset = normalizedName and Icons[normalizedName]
...
function Creator.IsHttpUrl(value)
	return typeof(value) == "string" and string.match(string.lower(value), "^https?://") ~= nil
end

function Creator.CanManageFolders()
	return type(isfolder) == "function" and type(makefolder) == "function"
end

function Creator.CanDownloadAssets()
	return type(Creator.Request) == "function"
...
function Creator.Image(Img, Name, Corner, Folder, Type, IsThemeTag, Themed, ThemeTagName)
	...
	local ImageFrame = New("Frame", {
		...
		New("ImageLabel", {
			...
			BackgroundTransparency = 1,
			ScaleType = "Crop",
			ThemeTag = (IconData or Themed) and {
	...
	if IconData then
		ImageFrame.ImageLabel.Image = IconData[1]
		ImageFrame.ImageLabel.ImageRectOffset = IconData[2].ImageRectPosition
		ImageFrame.ImageLabel.ImageRectSize = IconData[2].ImageRectSize
	elseif Creator.IsHttpUrl(Img) then
		warn("[ WindUI.Creator ] Remote image URLs are not supported in Roblox Studio/client contexts. Use a bundled icon or an rbxassetid instead: " .. Img)
		ImageFrame.Visible = false
	elseif Img == "" or Img == nil then
		ImageFrame.Visible = false
	else
