...
function OpenButton.New(Window)
    ...
    local DragModule = Creator.Drag(Container, {Drag})
    
    
    function OpenButtonMain:Visible(v)
        if v then
            Container.Visible = true
            Tween(Container, .25, {Position = UDim2.new(0.5,0,0,6+44/2)}, Enum.EasingStyle.Quad, Enum.EasingDirection.Out):Play()
        else
            local HideTween = Tween(Container, .25, {Position = UDim2.new(0.5,0,0,-(6+44/2))}, Enum.EasingStyle.Quad, Enum.EasingDirection.In)

            Creator.AddSignal(HideTween.Completed, function()
                if not OpenButtonMain.IsVisible then
