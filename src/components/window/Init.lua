...
-- local ConfigManager = require("../../config/Init")   edit
...
return function(Config)
    ...
    if Window.Folder and Creator.CanManageFolders() then
    ...
    if Window.Folder and ConfigManager then 
    ...
    if Window.User then
        ...
        local UseSemiAnonymous = Window.User.SemiAnonymous and not Window.User.Anonymous
        local SemiAnonymousName= Players.LocalPlayer.Name:sub(1,8) .. string.rep("*",#Players.LocalPlayer.Name - 8)
        ...
        UserIcon = New("TextButton", {
            ...
            Creator.NewRoundFrame(Window.UICorner-(Window.UIPadding/2), "Squircle", {
                ...
                New("Frame", {
                    ...
                    New("TextLabel", {
                        Text = (Window.User.Anonymous or Window.User.SemiAnonymous) and "Anonymous" or Players.LocalPlayer.DisplayName,
                        TextSize = 17,
                        ThemeTag = {
                            TextColor3 = "Text",
                        },
                    ...
                    New("TextLabel", {
                        Text = UseSemiAnonymous and SemiAnonymousName or Window.User.Anonymous and "Anonymous" or Players.LocalPlayer.Name,
                        TextSize = 15,
                        TextTransparency = .6,
                        ThemeTag = {
                            TextColor3 = "Text",
        ...
        if Window.User.Callback then
            ...
            Creator.AddSignal(UserIcon.MouseEnter, function()
                Tween(UserIcon.UserIcon, .2, {ImageTransparency = .95}):Play()
                Tween(UserIcon.Outline, .2, {ImageTransparency = .85}):Play()
            end)
            Creator.AddSignal(UserIcon.MouseLeave, function()
                Tween(UserIcon.UserIcon, .2, {ImageTransparency = 1}):Play()
                Tween(UserIcon.Outline, .2, {ImageTransparency = 1}):Play()
            end)
        end
    end
    ...
    if typeof(Window.Background) == "string" and BGVideo then
        if Creator.IsHttpUrl(BGVideo) then
            warn("[ WindUI.Window.Background ] Remote video URLs are not supported in Roblox Studio/client contexts. Use an uploaded video asset instead: " .. BGVideo)
        else
            IsVideoBG = true
    ...
    elseif BGImageUrl then
        warn("[ WindUI.Window.Background ] Remote image URLs are not supported in Roblox Studio/client contexts. Use an uploaded image asset instead: " .. BGImageUrl)
    ...
    Window:CreateTopbarButton("Minimize", "minus", function() 
        ...
        task.spawn(function()
            ...
            if not Window.IsPC and Window.IsOpenButtonEnabled then
                -- OpenButtonContainer.Visible = true     -- commented cuz dumb ftgs added it, even tho the function does exactly the same
