...
function NotificationModule.New(Config)
    local Notification = {
        ...
        UIElements = {},
        CanHover = Config.CanHover ~= false,
        Closed = false,
        Hovered = false
    }
...
Notification.CanClose = true
    ...
    if Notification.CanHover then
        Creator.AddSignal(MainContainer.MouseEnter, function()
            Notification.Hovered = true
        end)
        Creator.AddSignal(MainContainer.MouseLeave, function()
    ...
    task.spawn(function()
        ...
        if Notification.Duration then
            ...
            local MainTween = Tween(Main.DurationFrame.Frame, Notification.Duration, {Size = UDim2.new(0,0,1,0)}, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut)
            MainTween:Play()
            local timeLeft = Notification.Duration
            while timeLeft > 0 do
                local dt = task.wait()

                if Notification.Hovered then
